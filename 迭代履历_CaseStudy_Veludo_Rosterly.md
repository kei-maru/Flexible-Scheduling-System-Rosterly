...
## 5.1 2026-04-12 追加更新：登录文案/利用規約与租户软删除

关键内容：
- 登录与规约：
  - System B 登录页清理中文文案，统一为日语口径。
  - `dashboard/terms` 替换为最新版 `Rosterly 利用規約` 全文（2026-04-01 施行版）。
- 店铺消息中心：
  - 店铺管理员端由“铃铛弹层”收敛为独立消息中心页面。
  - 结构升级为两级：消息列表页（公告 + 通报）与消息详情页。
- 租户软删除与恢复：
  - `Tenant` 新增 `deleted_at / recoverable_until / deletion_requested_by`。
  - 管理台新增“删除申请 + 30 天内恢复”操作。
  - 删除状态下公开预约入口与可用 API 自动关闭，邀请链接同步失效。

阶段结论：
- 完成“规约口径一致化 + 管理动作可逆化”的治理升级。
- 通过软删除恢复窗口，降低误删风险并提升租户生命周期可运营性。
