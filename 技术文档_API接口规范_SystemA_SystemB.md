...
1. 店铺管理员消息入口变更
- 店铺管理员端将原顶部铃铛通知收敛为独立消息中心页面（两级结构：列表页/详情页）。
- 列表统一聚合“全体公告 + 预约通报”，详情页按类型渲染。

2. 租户软删除对公开预约接口的影响
- 当租户进入软删除状态（`tenant.deleted_at` 非空）后，公开预约链路将视同租户不存在：
  - `GET /dashboard/book/<tenant_slug>/`
  - `GET /dashboard/book/<tenant_slug>/api/availability/`
  - `POST /dashboard/book/<tenant_slug>/api/create/`
- 行为口径：返回 `tenant not found` 或等价不可用状态，避免继续对外承接预约。
