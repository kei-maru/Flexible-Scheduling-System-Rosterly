...
class IntegrationAvailabilityView(APIView):
    ...
    def get(self, request):
        """
        统一查询接口：
        mode='search': Guest 搜索用 (返回合并后的纯空闲时间)
        mode='calendar_admin': Cast 后台用 (返回 FullCalendar 需要的 events，包含 ID 和 Bookings)
        mode='raw': 原始数据
        """
        ...
        if resource_id_raw:
            try:
                target_uuid = UUID(resource_id_raw)
            except ValueError:
                try:
        ...
        if not target_uuid and mode in ['search', 'calendar_admin']:
             return Response({'error': 'resource_id required'}, status=400)

        start_dt, end_dt = None, None
        if start_str and end_str:
            try:
                # [关键修复] 强制将 Naive 时间解析为 JST
        ...
        if mode == 'search':
            ...
            slots = calculate_free_slots(target_uuid, start_dt, end_dt, request.tenant)
            return Response([{'start': s['start'], 'end': s['end'], 'status': 'AVAILABLE'} for s in slots])

        elif mode == 'calendar_admin':
            if not start_dt or not end_dt: return Response({'error': 'Time range required'}, status=400)
            events = generate_admin_calendar_events(target_uuid, start_dt, end_dt, request.tenant)
            return Response(events)

        else: # Raw
            queryset = Availability.objects.filter(resource__tenant=request.tenant)
            if target_uuid: queryset = queryset.filter(resource__id=target_uuid)
            if start_dt: queryset = queryset.filter(start_time__gte=start_dt)
            if end_dt: queryset = queryset.filter(end_time__lte=end_dt)
            return Response([{
                'id': str(a.id),
                'resource_id': a.resource.external_id,
                'start': a.start_time, 
                'end': a.end_time,
    ...
    def _handle_single(self, request, resource_uuid):
        ...
        avail = Availability.objects.create(resource=resource, start_time=start_dt, end_time=end_dt)
    ...
    def delete(self, request, pk=None):
        ...
        # 确保只能删除该租户下的排班
        avail = get_object_or_404(Availability, id=avail_id, resource__tenant=request.tenant)
