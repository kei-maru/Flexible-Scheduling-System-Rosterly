...
- `Resource`
  ...
  - `is_active` は「公開予約の対象として表示するか」を表す。
  - `SaaSUser.is_active` はアカウントの有効状態であり、両者は別の意味を持つ。

- `ResourceProfile`
  - 公開プロフィール、表示順、タグ、avatar URL、対応コース設定、コンテンツ確認状態を持つ。
  - System A が管理する Cast は同期時に `platform_terms_agreed=true` を送信する。
...
- `Booking`
  ...
  - status は主に `CONFIRMED`、`CANCELLED`、`CANCELLED_BY_ADMIN`、`COMPLETED` を使う。
  - `CANCELLED_BY_ADMIN` は、顧客メールが未設定でも管理者が Shifts & Order 詳細から予約を停止するための状態である。
...
Availability API は raw Availability をそのまま返さない。
各 `CONFIRMED` Booking の開始 30 分前から終了 30 分後までを使用不可区間として Availability から切り取り、残った区間のみを返す。24 時間前予約制限と店舗の予約受付期間も API 側で裁断する。

ブラウザキャッシュは無効化し、フロント側では古い非同期レスポンスが新しい結果を上書きしないよう request sequence を管理する。予約確認画面を開く直前にも空き枠を再取得し、選択時間がまだ有効であることを確認する。予約成功後は同じ Resource の空き枠を再読込する。

顧客メールが未入力の場合、メール送信済みとは表示せず、`予約が完了しました。担当者からの連絡をお待ちください。` と案内する。
...
- Booking 操作
  - スタッフは自分の担当予約を完了できる。
  - 管理者は Shifts & Order の注文詳細から `CONFIRMED` 予約を `CANCELLED_BY_ADMIN` に変更できる。
- identity / role 同期

Cast / Resource 同期では次の責務境界を守る。

- System A の公開チェックは System B `Resource.is_active` に同期する。
- `Resource.is_active=false` は非公開を意味し、退職やアカウント停止を意味しない。
- System B の Staff 一覧も `Resource.is_active` を予約対象チェックとして表示・更新する。
- Resource 自動 binding は `SaaSUser.is_active` で既存の `Resource.is_active` を上書きしない。
- System A 管理 Profile は `platform_terms_agreed=true` を同期する。
- Profile 保存は `display_order` を変更せず、表示順は管理者の drag-and-drop 操作だけで更新する。

現時点の System A では ADMIN が Cast 権限を内包し、管理者にも CastProfile が自動作成される場合がある。全件同期コマンドはその Profile も対象にするため、管理専用アカウントは `is_active=false` にする。将来は ADMIN と CAST 資格を独立させることが改善候補である。
